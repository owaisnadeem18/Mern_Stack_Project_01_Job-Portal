    ...
    <div className="border border-gray-400 p-3 rounded-lg">
      <div className="flex items-center justify-between">
        <p className="text-gray-200 font-semibold">
          ...
          <Badge variant={"ghost"} className="text-sm font-bold bg-gray-700">
            {" "}
            {JobPostedTotalTime(job?.createdAt) === 0
              ? "Today" : JobPostedTotalTime(job?.createdAt) === 1 ? `${JobPostedTotalTime(job?.createdAt)} day ago` :
               `${JobPostedTotalTime(job?.createdAt)} days ago`}{" "}
          </Badge>{" "}
        </p>
          ...
          <div>
            {job?.company?.name ? (
              <h6 className="text-white font-semibold">
                <Badge variant={"ghost"}>{job?.company?.name}</Badge>
              </h6>
            ) : (
              ""
            )}
      ...
      <div>
        <h4 className="text-white text-lg font-bold">{job?.title}</h4>
      ...
      <div className="flex gap-2 flex-wrap">
        <Button
          variant={"outline"}
          className="mt-4 text-[12px] cursor-pointer py-1 px-4 rounded-lg"
        >
